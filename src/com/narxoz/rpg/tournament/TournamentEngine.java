...
public class TournamentEngine {
    ...
    public TournamentResult runTournament() {
        ...
        while (hero.isAlive() && opponent.isAlive() && round < maxRounds){
            ...
            actionQueue.enqueue(new HealCommand(hero, healAmount));
            actionQueue.enqueue(new DefendCommand(hero, dodgeBoost));
        ...
        if(hero.isAlive() && !opponent.isAlive()){
            ...
            result.addLine("[Result] Max rounds reached (" + maxRounds + "). Winner chosen by remaining HP.");
        }else {
            winner = "Draw";
            result.addLine("[Result] Both fighters fell. Draw");
        }
